...
const tabConfig = {
  gamecards: {
    ...
    idField: 'id',
    titleField: 'content',
    searchFields: ['id', 'type', 'content', 'category', 'estimatedTime', 'safetyNotes'],
    filters: {
      type: 'type',
      intensity: 'intensity',
      category: 'category',
    ...
    fields: [
      { name: 'id', label: 'ID', type: 'text', placeholder: 'Auto-generated when blank' },
      { name: '_source', label: 'Deck Source', type: 'select', options: ['main', 'level1', 'level2', 'level3', 'level4', 'level5'] },
      { name: 'type', label: 'Type', type: 'select', options: ['truth', 'dare', 'challenge', 'fantasy', 'roleplay'], required: true },
      { name: 'content', label: 'Content', type: 'textarea', required: true },
      { name: 'intensity', label: 'Intensity', type: 'select', options: [1, 2, 3, 4, 5], parser: Number, required: true },
      { name: 'category', label: 'Category', type: 'select', options: ['communication', 'physical', 'emotional', 'playful', 'intimate'], required: true },
      { name: 'isPremium', label: 'Premium', type: 'checkbox' },
      { name: 'estimatedTime', label: 'Timer / Estimated Time', type: 'text', placeholder: 'N/A, 30 sec, 5 min' },
      { name: 'requires', label: 'Required Props', type: 'array', placeholder: 'ice, scarf, candles' },
  ...
  kinks: {
    ...
    idField: 'id',
    titleField: 'title',
    searchFields: ['id', 'slug', 'title', 'titleEs', 'description', 'descriptionEs', 'category', 'tier', 'tags'],
    filters: {
      type: 'tier',
    ...
    fields: [
      ...
      { name: 'tags', label: 'Tags', type: 'array', placeholder: 'touch, intimacy, playful' },
      { name: 'category', label: 'Category', type: 'text', required: true },
      { name: 'intensityScale', label: 'Intensity', type: 'select', options: [1, 2, 3], parser: Number, required: true },
      { name: 'tier', label: 'Tier', type: 'select', options: ['soft', 'naughty', 'xxx'], required: true },
    ],
  },
  'conversation-starters': {
    ...
    idField: 'id',
    titleField: 'question',
    searchFields: ['id', 'category', 'question', 'context', 'followUps', 'tags'],
    filters: {
      type: '_source',
      intensity: 'intensity',
      category: 'category',
    ...
    fields: [
      { name: 'id', label: 'ID', type: 'text', placeholder: 'Auto-generated when blank' },
      { name: '_source', label: 'Prompt Pack', type: 'select', options: ['dateNight', 'gettingToKnow', 'loveLanguages', 'relationship', 'spicy'] },
      { name: 'category', label: 'Category', type: 'text', required: true },
      { name: 'intensity', label: 'Intensity', type: 'select', options: [1, 2, 3, 4, 5], parser: Number, required: true },
      { name: 'question', label: 'Question', type: 'textarea', required: true },
      { name: 'followUps', label: 'Follow-Ups', type: 'array', placeholder: 'One follow-up per line or comma-separated' },
      { name: 'context', label: 'Context', type: 'textarea' },
      { name: 'tags', label: 'Tags', type: 'array', placeholder: 'flirty, playful, intimacy' },
    ],
  },
};
...
function updateFilterOptions() {
  ...
  updateSelect(elements.filterCategory, config.filters.category, 'All Categories');

  const intensities = uniqueValues(config.filters.intensity);
  elements.filterIntensity.innerHTML = '<option value="">All Intensities</option>';
...
function uniqueValues(fieldName) {
  if (!fieldName) return [];
  return Array.from(new Set(state.items.map((item) => item[fieldName]).filter((value) => value !== undefined && value !== '')))
...
function applyFilters() {
  ...
  state.filteredItems = state.items.filter((item) => {
    const matchesSearch = !query || config.searchFields.some((field) => {
    ...
    const matchesIntensity = !intensity || String(item[config.filters.intensity]) === intensity;
    const matchesCategory = !category || String(item[config.filters.category]) === category;
...
function renderList() {
  if (state.filteredItems.length === 0) {
    elements.contentList.innerHTML = '<div class="empty-state">No matching content found.</div>';
    return;
  }

  elements.contentList.innerHTML = state.filteredItems.map(renderCard).join('');

  elements.contentList.querySelectorAll('[data-action="edit"]').forEach((button) => {
    button.addEventListener('click', () => openEditModal(findItem(button.dataset.id)));
  });

  elements.contentList.querySelectorAll('[data-action="delete"]').forEach((button) => {
    button.addEventListener('click', () => openDeleteModal(findItem(button.dataset.id)));
  });
}
...
function renderCard(item) {
  ...
  const title = item[config.titleField] || item.title || item.question || item.content || id;
...
function renderBadges(item) {
  ...
  if (item.intensity || item.intensityScale) badges.push(['badge-intensity', `Intensity ${item.intensity || item.intensityScale}`]);
  if (item.category) badges.push(['badge-category', item.category]);
  if (typeof item.isPremium === 'boolean') badges.push([item.isPremium ? 'badge-premium' : 'badge-free', item.isPremium ? 'Premium' : 'Free']);

  return badges.map(([className, label]) => `<span class="badge ${className}">${escapeHtml(label)}</span>`).join('');
}

function renderMeta(item) {
  const meta = [];
  if (item.estimatedTime) meta.push(`Timer: ${item.estimatedTime}`);
  if (Array.isArray(item.requires) && item.requires.length) meta.push(`Requires: ${item.requires.join(', ')}`);
  if (Array.isArray(item.tags) && item.tags.length) meta.push(`Tags: ${item.tags.join(', ')}`);
  if (Array.isArray(item.followUps) && item.followUps.length) meta.push(`${item.followUps.length} follow-ups`);
...
function openEditModal(item = null) {
  ...
  elements.modalTitle.textContent = item ? `Edit ${getConfig().label}` : `Create ${getConfig().label}`;
...
function renderForm(item) {
  return getConfig().fields.map((field) => renderField(field, item[field.name])).join('');
}
...
function collectFormData() {
  ...
  config.fields.forEach((field) => {
    ...
    data[field.name] = field.parser && rawValue !== '' ? field.parser(rawValue) : rawValue;
  });

  if (state.activeTab === 'gamecards') {
    data._arrayName = data._source === 'main'
      ? (data.isPremium ? 'PREMIUM_CARDS' : 'FREE_CARDS')
...
function bindEvents() {
  ...
  elements.saveButton.addEventListener('click', () => elements.editForm.requestSubmit());
