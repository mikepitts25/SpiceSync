...
async function commitChanges(message) {
  try {
    ...
    try {
      ...
      return {
        success: true,
        message: "Changes committed locally (push to GitHub failed)",
      };
    }
  } catch (err) {
...
app.post("/api/gamecards", async (req, res) => {
  try {
    ...
    if (cards.some((c) => c.id === newCard.id)) {
      return res
        .status(400)
        .json({ success: false, error: "Card with this ID already exists" });
    }
...
app.get("/api/kinks", (req, res) => {
  try {
    ...
    res.json({ success: true, data: kinks, qa: analyzeKinks(kinks) });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});

app.get("/api/kinks/qa", (req, res) => {
  try {
    const kinks = loadKinks();
...
app.post("/api/kinks", async (req, res) => {
  try {
    ...
    if (kinks.some((k) => k.id === newKink.id)) {
      return res
        .status(400)
        .json({ success: false, error: "Kink with this ID already exists" });
    }
  ...
  try {
    ...
    if (starters.some((s) => s.id === newStarter.id)) {
      return res
        ...
        .json({
          success: false,
          error: "Conversation starter with this ID already exists",
        });
    }
    ...
    const gitResult = await commitChanges(
      `Add conversation starter: ${newStarter.id}`,
    );
...
app.put("/api/conversation-starters/:id", async (req, res) => {
  try {
    ...
    if (index === -1) {
      return res
        .status(404)
        .json({ success: false, error: "Conversation starter not found" });
    }
...
app.listen(PORT, () => {
  ...
  console.log(
    `📁 Data directory: ${path.join(__dirname, "../apps/mobile/data")}`,
  );
});
