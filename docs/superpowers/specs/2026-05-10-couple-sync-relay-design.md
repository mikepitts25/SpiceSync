...
- The Cloudflare relay stores only encrypted event payloads and minimal routing metadata.
...
Use Cloudflare Worker + D1 for the MVP backend. D1 gives the relay a simple ordered event log, invite table, expiration metadata, and cleanup path without standing up a traditional server or account database.
...
1. Implement relay schema and Worker API locally.
...
8. Add cleanup jobs for expired invites and old events.
