...
const ROMANCE_HINTS = new Set([
  'romance','romantic','date','kissing','cuddling','massage','aftercare','affirmations','quality-time','compliments','hand-holding','eye-contact','slow-dance','sensual'
]);

const SPICY_HINTS = new Set([
  'anal','training','impact','bondage','restraint','roleplay','control','domination','submission','group','exhibition','public','voyeur','chastity','cuckold','pegging','discipline','edging','denial'
]);
...
function collapseSequences(items: any[]): any[] {
  ...
  for (const it of items) {
    const key = (it.slug ? baseSlug(it.slug) : baseTitle(it.title || '')).toLowerCase();
    if (!map.has(key)) {
      ...
      if (!first.description || STEP_SUFFIX.test(String(it.title || ''))) {
        first.description = first.description || 'A progressive, consensual practice—start at a comfortable level and adjust gradually together.';
      }
      map.set(key, first);
    } else {
...
function defaultTier(k: any): Tier {
  ...
  const tags = (Array.isArray(k.tags) ? k.tags : []).map((t: string) => String(t).toLowerCase());
  ...
  const hasSpice = tags.some(t => SPICY_HINTS.has(t)) || /(impact|anal|group|control|bondage|xxx|hard)/i.test(cat);
  if (intensity <= 1 && !hasSpice) return 'romance';

  // Roleplay / sensory / light → soft by default
  if (cat.includes('roleplay') || cat.includes('sensory') || cat.includes('light')) return 'soft';

  // Spicier toys/groups/control escalate by intensity
  if (/(props|toys|group|environment|communication|control|bondage|impact)/i.test(cat) || hasSpice) {
...
export function useKinks(lang:'en'|'es'='en') {
  ...
  const kinks: KinkItem[] = collapsed.map(k => ({ ...k, tier: defaultTier(k) }));
